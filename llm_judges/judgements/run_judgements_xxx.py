...
def replace_numbers_with_xxx(input_string):
    """
    This function takes a string and replaces every number with 'XXX'.

    :param input_string: str: The input string
    :return: str: The modified string with numbers replaced by 'XXX'
    """
    return re.sub(r"\d+", "X", input_string)
...
def main(
    ...
    j = 0
    # for (model_1_name, model_2_name), dataset_dict in reversed(config_dict["full_dict"].items()):
            ...
            for idx, idx_data in idx_dict.items():
                ...
                if idx_comparisons is not None:
                    for i in range(len(idx_comparisons)):
                        ...
                        idx_comparisons[i]["model_1_opinion_str"] = (
                            replace_numbers_with_xxx(model_1_opinion_str)
                        )
                        idx_comparisons[i]["model_2_opinion_str"] = (
                            replace_numbers_with_xxx(model_2_opinion_str)
                        )
